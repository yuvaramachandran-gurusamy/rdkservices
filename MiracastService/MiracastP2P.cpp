...
static MiracastP2P *g_MiracastP2P = NULL;
...
static void iarmEvtHandler(const char *owner, IARM_EventId_t eventId, void *data, size_t len)
{
    if (strcmp(owner, IARM_BUS_NM_SRV_MGR_NAME) == 0)
    {
        ...
        g_MiracastP2P->evtHandler(p2pEventId, data, len);
    }
}
...
void MiracastP2P::wfdInit(MiracastServiceNotifier *notifier)
{
    MIRACASTLOG_TRACE("Entering..");
...
void MiracastP2P::setWiFiDisplayParams(void)
{
    MIRACASTLOG_TRACE("Entering..");
...
void MiracastP2P::resetWiFiDisplayParams(void)
{
    m_isWiFiDisplayParamsEnabled = false;
}

void MiracastP2P::applyWFDSinkDeviceName(void)
{
    MIRACASTLOG_TRACE("Entering..");
    std::string command, retBuffer;
